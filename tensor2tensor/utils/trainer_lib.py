...
def create_hparams(
    hparams_set, hparams_overrides_str="", data_dir=None, problem_name=None, hparams_path=None
):
    ...
    if hparams_overrides_str:
        tf.logging.info("Overriding hparams in %s with %s", hparams_set, hparams_overrides_str)
            ...
            for key in sorted(new_hparams.values().keys()):
                if hasattr(hparams, key):  # Overlapped keys
                    ...
                    if value != new_value:  # Different values
                        tf.logging.info("Overwrite key %s: %s -> %s" % (key, value, new_value))
...
def is_cloud_async_distributed():
    return "chief" in json.loads(os.environ.get("TF_CONFIG", "{}")).get("cluster", {})
...
def create_run_config(
    ...
    if not use_tpu:
        ...
        if use_distribution_strategy:
            tf.logging.info(
                "Configuring MirroredStrategy DistributionStrategy to replicate the " "model."
            )
    ...
    if use_tpu or use_tpu_estimator:
        ...
        if decode_hparams and run_config.tpu_config:
            decode_hparams.add_hparam(
                "iterations_per_loop", run_config.tpu_config.iterations_per_loop
            )
...
class HookContext(collections.namedtuple("HookContext", ["estimator", "problem", "hparams"])):
    pass
...
class T2TExperiment(object):
    ...
    def __init__(
        self, estimator, hparams, train_spec, eval_spec, use_validation_monitor, decode_hparams=None
    ):
    ...
    def forai_train_and_eval(self):
        try:
            steps = self._estimator.get_variable_value("global_step")
        except Exception:
            steps = 0

        eval_steps = self._hparams.eval_freq_in_steps
        mlperf_log.transformer_print(key=mlperf_log.TRAIN_LOOP)
        for i in range(steps, self._train_spec.max_steps, eval_steps):
            self._hparams.gs = steps
            self._estimator.train(
                self._train_spec.input_fn, steps=eval_steps, hooks=self._train_spec.hooks
            )
    ...
    def train_and_evaluate(self):
        if self._use_validation_monitor:
            tf.logging.warning(
                "EvalSpec not provided. Estimator will not manage "
    ...
    def train_eval_and_decode(self):
        ...
        for i in range(0, self._train_spec.max_steps, eval_steps):
            mlperf_log.transformer_print(key=mlperf_log.TRAIN_EPOCH, value=i // eval_steps)
            ...
            if packed_dataset:
                problem = registry.problem(self._hparams.problem.name.replace("_packed", ""))
                p_hparams = problem.get_hparams(self._hparams)
                self._hparams.problem = problem
                self._hparams.problem_hparams = p_hparams
            ...
            if self._hparams.mlperf_mode and d_hparams.mlperf_success:
                mlperf_log.transformer_print(key=mlperf_log.RUN_STOP, value={"success": "true"})
                break

        d_hparams = self._decode_hparams
        if self._hparams.mlperf_mode and not d_hparams.mlperf_success:
            mlperf_log.transformer_print(key=mlperf_log.RUN_STOP, value={"success": "false"})
    ...
    def continuous_eval(self):
        ...
        for ckpt_path in next_checkpoint(self._hparams.model_dir, self._hparams.eval_timeout_mins):
            # Skip zero'th step.
    ...
    def test(self):
        ...
        self._estimator.evaluate(self._eval_spec.input_fn, steps=1, hooks=self._eval_spec.hooks)
    ...
    def continuous_decode_on_eval_data(self):
        ...
        for ckpt in ckpt_generator:
            ...
            mlperf_log.transformer_print(key=mlperf_log.EVAL_START)
            self.decode(dataset_split=tf.estimator.ModeKeys.EVAL, checkpoint_path=checkpoint_path)
            d_hparams = self._decode_hparams
            if self._hparams.mlperf_mode and d_hparams.mlperf_success:
                mlperf_log.transformer_print(key=mlperf_log.RUN_STOP, value={"success": "true"})
                break
...
def create_experiment(
    ...
    if schedule == "continuous_train_and_eval" and min_eval_frequency:
        tf.logging.warn("ValidationMonitor only works with " "--schedule=train_and_evaluate")
    use_validation_monitor = schedule == "train_and_evaluate" and min_eval_frequency
    ...
    train_hooks = tf.contrib.learn.monitors.replace_monitors_with_hooks(train_hooks, estimator)
    eval_hooks = tf.contrib.learn.monitors.replace_monitors_with_hooks(eval_hooks, estimator)

    train_spec = tf.estimator.TrainSpec(train_input_fn, max_steps=train_steps, hooks=train_hooks)
