...
def optimize(loss, learning_rate, hparams, use_tpu=False):
    ...
    non_trainable_variables = list(set(tf.global_variables()) - set(tf.trainable_variables()))
    log_variable_sizes(
        non_trainable_variables, tag="Non-trainable variables", verbose=hparams.summarize_vars
    )
    ...
    diet_vars = [v for v in tf.global_variables() if v.dtype == dtypes.float16_ref]
    log_variable_sizes(diet_vars, "Diet Variables", verbose=hparams.summarize_vars)
    ...
    if hparams.grad_noise_scale:
        tf.logging.info("Adding noise to gradients, noise scale: %0.5f", hparams.grad_noise_scale)
...
class ConditionalOptimizer(tf.train.Optimizer):
    ...
    def __init__(
        self, optimizer_name, lr, hparams, use_tpu=False
    ):  # pylint: disable=super-init-not-called
        tf.logging.info("Using optimizer %s", optimizer_name)

        mlperf_log.transformer_print(key=mlperf_log.OPT_NAME, value=optimizer_name, hparams=hparams)
        mlperf_log.transformer_print(
            key=mlperf_log.OPT_HP_ADAM_BETA1, value=hparams.optimizer_adam_beta1, hparams=hparams
        )
        mlperf_log.transformer_print(
            key=mlperf_log.OPT_HP_ADAM_BETA2, value=hparams.optimizer_adam_beta2, hparams=hparams
        )
        mlperf_log.transformer_print(
...
def weight_decay_and_noise(loss, hparams, learning_rate, var_list=None):
    ...
    weight_noise_ops = weight_noise(hparams.weight_noise, learning_rate, noise_vars)
...
def weight_noise(noise_rate, learning_rate, var_list):
    ...
    tf.logging.info(
        "Applying weight noise scaled by learning rate, " "noise_rate: %0.5f", noise_rate
    )
...
def get_variable_initializer(hparams):
    ...
    mlperf_log.transformer_print(
        key=mlperf_log.MODEL_HP_INITIALIZER_GAIN, value=hparams.initializer_gain, hparams=hparams
    )
