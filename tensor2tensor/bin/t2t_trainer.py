...
flags.DEFINE_integer("iterations_per_loop", 100, "Number of iterations in a TPU training loop.")
flags.DEFINE_bool("use_tpu", False, "Whether to use TPU.")
flags.DEFINE_bool(
    "use_tpu_estimator",
    False,
    "Whether to use TPUEstimator. " "This is always enabled when use_tpu is True.",
)
flags.DEFINE_bool("xla_compile", False, "Whether to use XLA to compile model_fn.")
flags.DEFINE_integer("xla_jit_level", -1, "GlobalJitLevel to use while compiling the full graph.")
flags.DEFINE_integer("tpu_infeed_sleep_secs", None, "How long to sleep the infeed thread.")
...
try:
    ...
    flags.DEFINE_string("schedule", "continuous_train_and_eval", "Method of Experiment to run.")
    flags.DEFINE_integer(
        "eval_steps",
        100,
...
flags.DEFINE_string("std_server_protocol", "grpc", "Protocol for tf.train.Server.")
...
flags.DEFINE_bool("cloud_mlengine", False, "Whether to launch on Cloud ML Engine.")
flags.DEFINE_string(
...
flags.DEFINE_string("autotune_objective", None, "TensorBoard metric name to optimize.")
flags.DEFINE_bool(
    "autotune_maximize", True, "Whether to maximize (vs. minimize) autotune_objective."
)
flags.DEFINE_integer("autotune_max_trials", 10, "Maximum number of tuning experiments to run.")
flags.DEFINE_integer(
...
def create_experiment_fn():
    return trainer_lib.create_experiment_fn(
        ...
        eval_early_stopping_metric_minimize=FLAGS.eval_early_stopping_metric_minimize,
...
def profile_context():
    if FLAGS.profile:
        with tf.contrib.tfprof.ProfileContext(
            "t2tprof", trace_steps=range(100), dump_steps=range(100)
        ) as pctx:
...
def save_metadata(hparams):
    ...
    if hasattr(FLAGS, "flags_into_string"):
        ...
        flags_str = "\n".join(["--%s=%s" % (name, str(f)) for (name, f) in flags_dict.items()])
        t2t_flags_str = None
...
def execute_schedule(exp):
    if not hasattr(exp, FLAGS.schedule):
        raise ValueError("Experiment has no method %s, from --schedule" % FLAGS.schedule)
...
def main(argv):
    ...
    mlperf_log.transformer_print(
        key=mlperf_log.RUN_SET_RANDOM_SEED, value=FLAGS.random_seed, hparams=hparams
    )
    ...
    hparams.add_hparam("eval_steps", FLAGS.eval_steps)
    hparams.add_hparam("schedule", FLAGS.schedule)
    hparams.add_hparam("warm_start_from", FLAGS.warm_start_from)
    hparams.add_hparam("std_server_protocol", FLAGS.std_server_protocol)
    hparams.add_hparam("eval_freq_in_steps", FLAGS.local_eval_frequency)
    hparams.add_hparam("eval_timeout_mins", FLAGS.eval_timeout_mins)
    ...
    for i in range(steps, FLAGS.train_steps, hparams.eval_freq_in_steps):
        hparams.gs = i
        estimator.train(train_input_fn, steps=hparams.eval_freq_in_steps)
